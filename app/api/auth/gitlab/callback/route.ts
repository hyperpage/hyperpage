...
import { validateOAuthState, getOAuthStateClearCookieOptions } from '@/lib/oauth-state-cookies';
  ...
  try {
    ...
    try {
      ...
      await tokenStorage.storeTokens(userId, PROVIDER_NAME, {
        ...
        scopes: tokenResponse.scope ? tokenResponse.scope.split(' ') : undefined,
        metadata: {
    ...
    const successUrl = new URL(process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000');
