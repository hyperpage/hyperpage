...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!code) {
      console.error(`${PROVIDER_NAME} OAuth callback: Missing authorization code`);
      return NextResponse.redirect(
    ...
    if (state) {
      try {
        ...
        if (!storedCookieValue) {
          console.error(`${PROVIDER_NAME} OAuth: Invalid state parameter - no cookie found`);
          return NextResponse.redirect(
        ...
        if (storedCookieValue !== state) {
          console.error(`${PROVIDER_NAME} OAuth: Invalid state parameter - state mismatch`);
          return NextResponse.redirect(
            `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}?error=${PROVIDER_NAME}_oauth_invalid_state`
          );
        }
        ...
        console.error(`${PROVIDER_NAME} OAuth: Cookie validation failed:`, cookieError);
        return NextResponse.redirect(
          `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}?error=${PROVIDER_NAME}_oauth_invalid_state`
        );
      }
    ...
    try {
      ...
      await tokenStorage.storeTokens(userId, PROVIDER_NAME, {
        ...
        scopes: typeof tokenResponse.scope === 'string' ? tokenResponse.scope.split(' ') : undefined,
        metadata: {
    ...
    const { name, value, options } = getOAuthStateClearCookieOptions(PROVIDER_NAME);
