...
export function useAuthOperations(initialTools: string[] = ["github", "gitlab", "jira"]) {
  ...
  const handleAuthSuccess = useCallback(async (tool: string) => {
    setTimeout(async () => {
  ...
  const isConfigured = useCallback((toolSlug: string): boolean => {
    return configuredTools[toolSlug] || false;
  }, [configuredTools]);

  const authenticate = useCallback(async (toolSlug: string): Promise<void> => {
  ...
  const disconnect = useCallback(async (toolSlug: string): Promise<void> => {
    updateToolState(toolSlug, { isLoading: true, error: null });
  ...
  const checkAuthStatus = useCallback(async (toolSlug: string): Promise<boolean> => {
    try {
