...
export class CacheInvalidationMiddleware {
  ...
  async applySmartCaching(
    ...
    const staleWhileRevalidate = Math.floor(
      (this.options.staleWhileRevalidate || 60000) / 1000,
    );
   ...
   */
  private async invalidateByRule(rule: InvalidationRule): Promise<void> {
    try {
      ...
      for (const key of cacheKeys) {
        if (typeof key === "string") {
          ...
          if (rule.invalidateRelated && !shouldInvalidate) {
            shouldInvalidate = this.isRelatedResource(
              key,
...
export const defaultCacheInvalidationMiddleware =
  new CacheInvalidationMiddleware();
...
export async function applySmartCaching(
  ...
  const middleware = options
    ? new CacheInvalidationMiddleware(options)
    : defaultCacheInvalidationMiddleware;
