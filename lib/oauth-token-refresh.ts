...
export class OAuthTokenRefresh {
   ...
   */
  async ensureValidTokens(userId: string, toolName: string): Promise<OAuthTokens | null> {
    try {
      ...
      if (this.tokenStorage.areExpired(tokens)) {
        ...
        const refreshedTokens = await this.refreshTokens(userId, toolName, tokens);

        if (refreshedTokens) {
          logger.info(`Successfully refreshed tokens for user ${userId}, tool ${toolName}`);
          return refreshedTokens;
        } else {
          logger.warn(`Failed to refresh tokens for user ${userId}, tool ${toolName}`);
      ...
      if (this.tokenStorage.shouldRefresh(tokens)) {
        logger.info(`Tokens need refresh (preemptive) for user ${userId}, tool ${toolName}`);

        try {
          const refreshedTokens = await this.refreshTokens(userId, toolName, tokens);
          if (refreshedTokens) {
            logger.info(`Successfully refreshed tokens preemptively for user ${userId}, tool ${toolName}`);
            return refreshedTokens;
          }
        } catch (error) {
          logger.warn(`Preemptive refresh failed for user ${userId}, tool ${toolName}, will use existing tokens for now:`, error);
        }
      }
      ...
      logger.error(`Error ensuring valid tokens for user ${userId}, tool ${toolName}:`, error);
      return null;
    }
  }
  ...
  private async refreshTokens(
    ...
    try {
      if (!currentTokens.refreshToken) {
        logger.warn(`No refresh token available for user ${userId}, tool ${toolName}`);
        return null;
      }

      // Check if refresh token is expired
      if (this.tokenStorage.isRefreshExpired(currentTokens)) {
        logger.warn(`Refresh token expired for user ${userId}, tool ${toolName}`);
        return null;
      }
      ...
      const newTokens = await this.performTokenRefresh(config, currentTokens.refreshToken);

      if (newTokens) {
        // Calculate expiry times if not provided
        const now = Date.now();
        const accessTokenExpiry = newTokens.expiresAt || (now + 3600 * 1000); // 1 hour default
        const refreshTokenExpiry = newTokens.refreshExpiresAt ||
          (currentTokens.refreshExpiresAt && currentTokens.refreshExpiresAt > now
        ...
        await this.tokenStorage.updateTokenExpiry(userId, toolName, accessTokenExpiry, refreshTokenExpiry);
      ...
      logger.error(`Token refresh failed for user ${userId}, tool ${toolName}:`, error);
      return null;
    }
  }
  ...
  private async performTokenRefresh(config: { clientId: string; clientSecret: string; tokenUrl: string; provider: string }, refreshToken: string): Promise<OAuthTokens | null> {
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(`Token refresh request failed for ${config.provider}: ${response.status}`, errorText);
      ...
      if (refreshData.error) {
        logger.error(`Token refresh response error for ${config.provider}:`, refreshData.error);
      ...
      logger.error(`Token refresh API call failed for ${config.provider}:`, error);
      return null;
    }
  }
   ...
   */
  async refreshExpiredTokens(): Promise<{ successful: number, failed: number }> {
    try {
      const expiredTokens = await this.tokenStorage.getExpiredTokens();
      let successful = 0;
      let failed = 0;

      logger.info(`Starting batch refresh of ${expiredTokens.length} expired tokens`);
      ...
      for (const { userId, toolName } of expiredTokens) {
        try {
          ...
          logger.error(`Batch refresh failed for user ${userId}, tool ${toolName}:`, error);
          failed++;
        }
      }

      logger.info(`Batch refresh completed: ${successful} successful, ${failed} failed`);
   ...
   */
  async getTokenExpiryInfo(userId: string, toolName: string): Promise<{
 ...
 */
export async function getValidTokens(userId: string, toolName: string): Promise<OAuthTokens | null> {
...
export async function performBackgroundTokenRefresh(): Promise<void> {
  ...
  if (result.successful > 0 || result.failed > 0) {
    logger.info(`Background token refresh completed: ${result.successful} refreshed, ${result.failed} failed`);
  }
}
