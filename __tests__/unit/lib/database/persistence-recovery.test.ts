...
describe("Persistence and Recovery System", () => {
  beforeAll(async () => {
    ...
    try {
      ...
      logger.debug("Test setup cleanup error (expected)", { error });
    }
  ...
  afterAll(async () => {
    ...
    try {
      ...
      // Log cleanup error at debug level - these are expected in cleanup scenarios
      logger.debug("Test teardown cleanup error (expected)", { error });
    }
  });
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      // Log cleanup error at debug level - these are expected in cleanup scenarios
      logger.debug("Test beforeEach cleanup error (expected)", { error });
    }
  });
  ...
  afterEach(async () => {
    ...
    try {
      ...
      // Log cleanup error at debug level - these are expected in cleanup scenarios
      logger.debug("Test afterEach cleanup error (expected)", { error });
    }
  });
  ...
  describe("Tool Configuration Persistence", () => {
    it("should persist and restore tool configuration across sessions", async () => {
      ...
      expect(retrieved[0].config).toMatchObject({
    ...
    it("should load persisted configurations from database", async () => {
      ...
      const configs = await testDb
        .select()
        .from(toolConfigs)
        .where(eq(toolConfigs.toolName, "jira"));
    ...
    it("should handle missing configuration gracefully", async () => {
      const configs = await testDb
        .select()
        .from(toolConfigs)
        .where(eq(toolConfigs.toolName, "non-existent-tool"));
    ...
    it("should update existing configuration", async () => {
      ...
      const updated = await testDb
        .select()
        .from(toolConfigs)
        .where(eq(toolConfigs.toolName, "gitlab"));
  ...
  describe("Rate Limit Persistence", () => {
    it("should persist and retrieve rate limit status", async () => {
      ...
      const persisted = await testDb
        .select()
        .from(rateLimits)
        .where(eq(rateLimits.platform, "github"));
    ...
    it("should handle platforms without specific limits gracefully", async () => {
      ...
      const persisted = await testDb
        .select()
        .from(rateLimits)
        .where(eq(rateLimits.platform, "gitlab"));
      expect(persisted.length).toBe(1);
  ...
  describe("Data Integrity and Recovery", () => {
    ...
    it("should handle concurrent data operations safely", async () => {
      ...
      const configs = await testDb
        .select()
        .from(toolConfigs)
        .where(eq(toolConfigs.toolName, `concurrent-test-0`));
      expect(configs.length).toBe(1);

      const allConcurrent = await testDb.select().from(toolConfigs);
      const concurrentConfigs = allConcurrent.filter((c) =>
        c.toolName?.startsWith("concurrent-test-"),
      );
