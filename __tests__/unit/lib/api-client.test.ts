...
describe("API Client Rate Limiting", () => {
  ...
  describe("makeRetryRequest", () => {
    ...
    const createToolConfig = (
      overrides: Partial<ToolRateLimitConfig> = {},
    ): ToolRateLimitConfig => ({
      detectHeaders: vi.fn().mockReturnValue({
        remaining: null,
        resetTime: null,
        retryAfter: null,
      }),
    ...
    it("should return successful response immediately", async () => {
      ...
      const response = await makeRetryRequest(mockUrl, mockOptions, {
    ...
    it("should retry on 429 status when shouldRetry indicates to", async () => {
      ...
      const responsePromise = makeRetryRequest(mockUrl, mockOptions, {
        rateLimitConfig: config,
        forceFetch: true,
      });
