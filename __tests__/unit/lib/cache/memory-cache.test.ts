...
describe("Memory Cache", () => {
  ...
  describe("Cache Key Generation", () => {
    it("should generate consistent hash keys", () => {
      const key1 = generateCacheKey("github", "issues", {
        state: "open",
        page: 1,
      });
      const key2 = generateCacheKey("github", "issues", {
        state: "open",
        page: 1,
      });
      const key3 = generateCacheKey("github", "issues", {
        state: "closed",
        page: 1,
      });
  ...
  describe("Concurrent Access", () => {
    it("should handle concurrent set and get operations", async () => {
      ...
      for (let i = 0; i < 100; i++) {
        promises.push(
          Promise.resolve().then(
            async () => await cache.set(`key${i}`, { value: i }, 60000),
          ),
        );
      }
      ...
      for (let i = 0; i < 50; i++) {
        promises.push(
          Promise.resolve().then(
            async () => await cache.get(`key${Math.floor(Math.random() * 50)}`),
          ),
        );
      }
