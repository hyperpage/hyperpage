...
describe("useRateLimit Hook", () => {
  ...
  it("should handle rate limit fetch failure", async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.error).toBe(
        "Failed to fetch rate limit status: API Error",
      );
      expect(result.current.isStale).toBe(false);
    });
  });
  ...
  it("should handle null rate limit response", async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.loading).toBe(false);
      expect(result.current.error).toBe(
        "Rate limit monitoring not supported for platform: nonexistent",
      );
      expect(result.current.isStale).toBe(false);
    });
  });
  ...
  it("should reset state when disabled after being enabled", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ enabled }) => useRateLimit("github", enabled),
      {
...
describe("useMultipleRateLimits Hook", () => {
  ...
  it("should load rate limits for multiple platforms", async () => {
    ...
    const { result } = renderHook(() =>
      useMultipleRateLimits(["github", "gitlab"], true),
    );
  ...
  it("should handle platform loading errors", async () => {
    ...
    await waitFor(() => {
      expect(result.current.errors.get("github")).toBe(
        "Failed to fetch rate limit status: GitHub API Error",
      );
      expect(result.current.errors.get("gitlab")).toBe(
        "Rate limit monitoring not supported for platform: gitlab",
      );
    });
  });
  ...
  it("should call refreshAll function", async () => {
    ...
    const { result } = renderHook(() =>
      useMultipleRateLimits(["github"], true),
    );
...
describe("Utility Functions", () => {
  describe("getRateLimitStatusColor", () => {
    it("should return correct colors for status", () => {
      expect(getRateLimitStatusColor("critical")).toBe(
        "text-red-600 dark:text-red-400",
      );
      expect(getRateLimitStatusColor("warning")).toBe(
        "text-yellow-600 dark:text-yellow-400",
      );
      expect(getRateLimitStatusColor("normal")).toBe(
        "text-green-600 dark:text-green-400",
      );
      expect(getRateLimitStatusColor("unknown")).toBe(
  ...
  describe("getRateLimitStatusBgColor", () => {
    it("should return correct background colors for status", () => {
      expect(getRateLimitStatusBgColor("critical")).toBe(
        "bg-red-100 dark:bg-red-900/20",
      );
      expect(getRateLimitStatusBgColor("warning")).toBe(
        "bg-yellow-100 dark:bg-yellow-900/20",
      );
      expect(getRateLimitStatusBgColor("normal")).toBe(
        "bg-green-100 dark:bg-green-900/20",
      );
      expect(getRateLimitStatusBgColor("unknown")).toBe(
