...
describe("Rate Limiting Coordination Tests", () => {
  ...
  describe("Individual Tool Rate Limiting", () => {
    it("should respect GitHub API rate limits", async () => {
      ...
      const finalRequest = await simulateAPICall(
        "github",
        "issues",
        { per_page: 30 },
        browser,
  ...
  describe("Rate Limit Recovery and Reset", () => {
    it("should recover from rate limits when reset time passes", async () => {
      ...
      const initialResult = await simulateAPICall(
        "github",
        "repos",
        {},
        browser,
      ...
      const recoveryResult = await simulateAPICall(
        "github",
        "repos",
        {},
        browser,
    ...
    it("should persist rate limit information across sessions", async () => {
      ...
      const persistedState = (await browser.getSessionData(
        "rate_limit_github",
      )) as RateLimitData;
      ...
      const updatedState = (await browser.getSessionData(
        "rate_limit_github",
      )) as RateLimitData;
  ...
  describe("Rate Limit User Experience", () => {
    ...
    it("should show rate limit status in real-time", async () => {
      ...
      const updatedDisplay = (await browser.getSessionData(
        "rate_limit_display",
      )) as {
        current: number;
        limit: number;
...
function setGitHubRateLimit(
  ...
  (browser as unknown as TestBrowserSession).setSessionData(
    "rate_limit_github",
    rateLimit,
  );
}
