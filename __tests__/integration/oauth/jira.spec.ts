...
describe("Jira OAuth Integration", () => {
  ...
  test.describe("Mock OAuth Processing", () => {
    ...
    test("should handle Jira OAuth errors", async ({ page }) => {
      await page.goto(
        `${baseUrl}/api/auth/jira/callback?error=access_denied&error_description=User denied access`,
      );
    ...
    test("should handle invalid authorization code", async ({ page }) => {
      await page.goto(
        `${baseUrl}/api/auth/jira/callback?code=invalid_code&state=mock_jira_state_token`,
      );
